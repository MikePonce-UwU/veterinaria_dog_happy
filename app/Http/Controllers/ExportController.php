...
*/
        ...
        return $pdf->stream('salesReport.' . now()->format('Y_m_d') . '.pdf'); // visualizar
        //$customReportName = 'salesReport_'.Carbon::now()->format('Y-m-d').'.pdf';
        //return $pdf->download($customReportName); //descargar

    }

    public function individualReporte(Sale $venta)
    {
        $pdf = PDF::loadView('pdf.reporte-individual', compact('venta'));
        return $pdf->stream('venta-individual.' . $venta->created_at->format('Y_m_d') . '.pdf');
    }
